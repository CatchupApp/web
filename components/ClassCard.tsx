...
const ClassCard = (props: ClassCardProps) => {
  ...
  return (
    <div className="w-64 h-48 text-white bg-purple-theme rounded-lg relative" onClick={props.onClassClick}>
        <div className="p-6">
        <div className="font-extrabold text-2xl">{classInfo.name}</div>
        <div className="font-semibold">{`Period ${classInfo.period}`}</div>
        <div className="font-bold text-xl" onClick={props.onTeacherClick}>{classInfo.teacher}</div>
      </div>
      {(lectures && lectures.length > 0) && 
        <div className="absolute bottom-0 w-full rounded-lg bg-black font-bold text-center p-2 text-sm" onClick={props.onLastLectureClick}>
