...
const TabBar = (props) => {
  ...
  const analysisColors = isFirstTab
    ? "bg-white text-black"
    : "bg-dark text-white";
  const transcriptColors = isFirstTab
    ? "bg-dark text-white"
    : "bg-white text-black";
  ...
  return (
    <div className="w-full h-10 bg-medium rounded-lg relative">
      ...
      `}</style>
      <div
        className={`flex items-center p-3 w-48 rounded-t-lg h-full font-bold absolute left-40 second-tab ${transcriptColors}`}
      ...
      <div
        className={`flex items-center p-3 w-40 rounded-t-lg h-full font-bold absolute top-0 left-0 ${analysisColors}`}
  ...
  return (
    <div className={`w-full ${props.className}`}>
      ...
      <div className="flex flex-row items-center mt-2">
        {paraInfo.keywords
          .slice(0, Math.min(paraInfo.keywords.length, 4))
          .map((value, index) => (
            <TagButton text={`#${value}`} className="mr-2" />
          ))}
      </div>
      <div
        className="flex flex-row items-center mt-2 text-sm"
        onClick={() => {
          toggleText(!textOpen);
        }}
      >
      ...
      {textOpen && <div className="mt-2 text-xs">{paraInfo.transcript}</div>}
    </div>
  );
};
...
const Transcript = (props) => {
  return (
    <div className="text-xs overflow-scroll">
      Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam et odio
      dapibus, faucibus mauris eget, porta nisl. Ut sodales, lorem nec mattis
      aliquam, odio odio varius purus, a luctus leo urna ut massa. Maecenas
      placerat semper orci. Mauris porttitor fermentum ipsum eget finibus.
      Curabitur sit amet maximus erat, nec fermentum nisi. Fusce ut fermentum
      nunc, ut mattis elit. Integer lectus nulla, bibendum quis ultricies id,
      consequat in eros. Nullam aliquet nibh ipsum, a laoreet felis molestie id.
      Pellentesque ultricies orci ligula, eget varius felis maximus a. In
      efficitur neque vitae odio luctus porta.
    </div>
  );
};
  ...
  render() {
    ...
    return (
      <div className="mr-2 bg-white rounded-lg flex-1 w-full h-full">
        <TabBar
          onTranscript={this.state.onTranscript}
          onTranscriptClick={this.onTranscriptClick}
          onAnalysisClick={this.onAnalysisClick}
        />
        <div className="p-4">
          {this.state.onTranscript && <Transcript />}
          {!this.state.onTranscript && (
            <AnalysisParagraph paragraphInfo={paraInfo} />
          )}
        </div>
      </div>
    );
  ...
  render() {
    return (
      <div className="w-full h-full p-4 bg-light flex flex-row">
        <LeftPage />
        <div className="ml-2 bg-white rounded-lg flex-1 w-full h-full overflow-hidden"></div>
      </div>
    );
  }
}
